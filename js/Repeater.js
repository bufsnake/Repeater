...
var CONTENT_TYPE = ["application/x-www-form-urlencoded", "multipart/form-data", "application/json", "application/xml", "application/javascript", "text/plain", "multipart/form-data; boundary=----WebKitFormBoundaryywB1rOnmpPkpqWtC"]
var ENC = 'DECODE'
...
function loadURL() {
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        ...
        chrome.runtime.sendMessage({ action: 'Get Session Storage', data: 'request_body_' + id },
            function(response) {
                // 判断SESSION中是否存在该链接
                ...
                chrome.runtime.sendMessage({ action: 'Get Session Storage', data: 'request_header_' + id },
                    function(response) {
                        ...
                        if (f_method != '') {
                            ...
                            for (var m = 0; m < METHODS.length; m++) {
                                if (METHODS[m].toUpperCase() === f_method.toUpperCase()) {
                                    document.querySelector('#METHOD > select')[
                                        m
                                    ].selected = true
                                    break
                                }
                        ...
                        if (f_request_body != '') {
                            ...
                            document.querySelector(
                                '#REQBODYRAW-TEXTAREA',
                            ).value = decodeURIComponent(f_request_body)
                        }
                    },
                )
            },
...
document.querySelector('#aboutBTN').addEventListener(
    ...
    function() {
        if (document.querySelector('#about').hidden) {
            ...
            document.querySelector('#aboutBTN').style.backgroundColor = 'rgb(241 243 244)'
        }
    },
    false,
)
...
document.querySelector('#REQBODY-ADD').addEventListener(
    ...
    `
        ...
        el.querySelector('#r-icon').querySelector('#r-hidden').value = XReqBodyCount
                ...
                for (var x = 0; x < flag_.length; x++) {
                    if (flag === parseInt(flag_[x].value)) {
                        if (event.target.value === '0') {
                            event.path[2][x * 6 + 4].type = 'text'
                        } else {
                            event.path[2][x * 6 + 4].type = 'file'
                        }
                        break
                    }
                }
...
// HEADER ADD Button
$(document.querySelector('#HEADER-ADD')).bind('click', function(
    event,
    key,
    val,
) {
...
document.querySelector('#sendRequest').addEventListener(
    ...
    function() {
            ...
            for (var j = 0; j < key.length; j++) {
                ...
                chrome.cookies.getAll({ url: url.origin }, function(cookies) {
                    ...
                    for (var i = 0; i < cookies.length; i++) {
                        chrome.cookies.remove({ name: cookies[i].name, url: url.origin },
                            function(response) {
                    ...
                    Object.keys(cookies).forEach(function(key) {
                        if (key.indexOf('X-X-X-X-X-X-Repeater_') !== -1) {
                            chrome.cookies.set({ value: cookies[key], url: url.origin },
                                function(response) {
                                    console.log('set cookie', response)
                                },
                            )
                        } else {
                            chrome.cookies.set({ name: key, value: cookies[key], url: url.origin },
                                function(response) {
                                    console.log('set cookie', response)
                                },
                            )
        ...
        chrome.webRequest.onBeforeSendHeaders.addListener(
            function(details) {
                ...
                for (var i = 0; i < key.length; i++) {
                    if (key[i].value === '' || key[i].value.toLowerCase() === 'content-type') {
                        continue
                    }
                    var flag = false
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function(
            ...
            if (new URL(tabs[0].url).origin !== new URL(document.querySelector('#URL').value).origin) {
                chrome.tabs.update({
                ...
                if (req_body_position === 1) {
                    chrome.tabs.query({ active: true, currentWindow: true }, function(
                        ...
                        for (var i = 0; i < type.length; i++) {
                            var type_ = types[parseInt(type[i].querySelector('select').value)]
                            if (type_ === 'TEXT') {
                                // 文本类型
                                var input = {
                                    type: 'text',
                        ...
                        // 复制devtools中的表单到当前网页
                        chrome.runtime.sendMessage({ action: 'Load Form', data: JSON.stringify(message) }, function(
                            response,
                        ) {
                    ...
                    chrome.runtime.sendMessage({ action: 'Send Raw Data', data: JSON.stringify(req) }, function(
                        response,
                    ) {
...
document.querySelector('#defaultOpenREQBODYRAW').addEventListener(
    'click',
    function() {
        CONTENT_TYPE = ["application/x-www-form-urlencoded", "text/plain", "application/json", "application/xml", "application/javascript", "multipart/form-data; boundary=----WebKitFormBoundaryywB1rOnmpPkpqWtC"]
        var select = document.querySelector('#ContentTypeSel')
        select.innerHTML = ''
        for (var i = 0; i < CONTENT_TYPE.length; i++) {
            var opt = document.createElement('option')
...
document.querySelector('#ContentTypeVal > select').addEventListener(
    ...
    function(event) {
        ...
        if (ContentType === 'multipart/form-data; boundary=----WebKitFormBoundaryywB1rOnmpPkpqWtC') {
            document.querySelector('#REQBODYRAW-TEXTAREA').value = `------WebKitFormBoundaryywB1rOnmpPkpqWtC
...
$(document).ready(function() {
    $("body").mouseup(function(e) {
        ...
        if (txt.toString().length >= 1 && ENC !== "DECODE") {
            ...
            console.log()
            if (e.target.value === undefined || e.target.value.indexOf(sources) === -1) {
                return
            }
